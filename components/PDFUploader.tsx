...
export default function PDFUploader() {
  ...
  const handleTailDragOver = (e: React.DragEvent) => {
    e.preventDefault();
    if (tailDropzoneRef.current) {
      tailDropzoneRef.current.classList.add('border-blue-500');
    }
  };

  const handleTailDragLeave = (e: React.DragEvent) => {
    e.preventDefault();
    if (tailDropzoneRef.current) {
      tailDropzoneRef.current.classList.remove('border-blue-500');
    }
  };
  ...
  const handleTailDrop = (e: React.DragEvent) => {
    ...
    if (files.length > 0) {
      if (files[0].type === 'image/png') {
        setTailFile(files[0]);
        setHasCachedTail(true);
        console.log(`已通过拖放选择尾部PNG文件: ${files[0].name}, 大小: ${(files[0].size / 1024 / 1024).toFixed(2)}MB`);
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col md:flex-row md:space-x-6">
        {/* 左侧区域 - 功能控制区 */}
        <div className="w-full md:max-w-md flex flex-col relative pb-16"> 
            ...
            <div>
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                <div 
                  ...
                  onDrop={handleDrop}
                  className="flex min-h-40 flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 p-6 transition-colors hover:border-blue-400"
                >
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                {hasCachedTail ? (
                  <div className="space-y-3">
                    ...
                    {tailFile && (
                      <p className="text-emerald-600 text-sm text-center">
                        已选择: {tailFile.name} <br />({(tailFile.size / 1024 / 1024).toFixed(2)}MB)
                      </p>
                    )}
                  </div>
                ) : (
                  <div 
                    ...
                    onDragLeave={handleTailDragLeave}
                  ...
                  >
                    ...
                    <p className="mb-2 text-center">拖拽PNG图片到此处，或</p>
                    <input 
                      type="file" 
                      accept="image/png" 
          ...
          {error && (
            <div className="mb-6 rounded-lg bg-red-50 p-4 text-red-700 border border-red-100">
        ...
        {/* 右侧区域 - 预览和下载 */}
        <div className="w-full md:pl-0 mt-8 md:mt-0 flex flex-col relative pb-16"> 
